...
import net.seensin.springdockerswarmmanagementapi.common.exception.dto.ErrorDto;
...
//            error.setTraceId(tracer.currentSpan().context().traceIdString());
        ...
        if (e instanceof DockerException) {
            DockerException template = (DockerException) e;
            error.setMessage(template.getMessage());
            error.setError(ErrorMap.ERROR);
            error.setStatus(template.getHttpStatus());
//            error.setTraceId(tracer.currentSpan().context().traceIdString());

            return new ResponseEntity(error, ErrorMap.getHttpStatus(template.getHttpStatus()));
        }
